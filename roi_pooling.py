...
# from theano.sandbox.cuda import cuda_ndarray_variable
...
class ROIPoolingOp(GpuOp, COp):
  ...
  def make_node(self, data, rois):
    data = basic_ops.as_cuda_ndarray_variable(data)
    rois = basic_ops.as_cuda_ndarray_variable(rois)
    assert data.ndim == 4
    assert rois.ndim == 2
...
class ROIPoolingGradOp(GpuOp, COp):
  ...
  def make_node(self, data, rois, argmaxes, out_grad):
    data = basic_ops.as_cuda_ndarray_variable(data)
    rois = basic_ops.as_cuda_ndarray_variable(rois)
    argmaxes = cuda_ndarray_variable(argmaxes)
    out_grad = cuda_ndarray_variable(out_grad)
    assert data.ndim == 4
    assert rois.ndim == 2
